...
# Usage:
#   bash scripts/inference/run_pipeline_gb.sh <R1> <R2> <RUN_NAME> <THRESH> [THREADS=8] [REF=data/refs/original.fasta]
...
python scripts/data_prep/extract_features.py \
  --bam "$BAM" \
  --out "$TSV" \
  --label -1 \
  --k 6 \
...
python scripts/inference/gb_predict_filter_pairs.py \
  --r1 "$R1" \
  --r2 "$R2" \
  --features "$TSV" \
  --model "$MODEL" \

...
# Usage:
#   bash scripts/inference/run_pipeline_rnnkmer.sh <R1> <R2> <RUN_NAME> [THRESH=0.5]
#
# Optional env:
...
python3 -m mitochime.deep_learning.make_seq_tsv_infer \
  --r1 "$R1" \
  --r2 "$R2" \
  --L 150 \
  --out "$SEQ_TSV"
...
python3 -m mitochime.deep_learning.predict_deep \
  --tsv "$SEQ_TSV" \
  --ckpt "$MODEL" \
